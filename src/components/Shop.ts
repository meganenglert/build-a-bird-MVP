...
export default class Shop extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    this.add(
      this.scene.add.text(40, 240, "Additional Colors", {
        fontSize: "18px",
        color: "#FFFFFF",
      })
    );
    this.add(
      this.scene.add.text(45, 415, "Additional Hats", {
        fontSize: "18px",
        color: "#FFFFFF",
      })
    );
    this.add(
      this.scene.add.text(80, 260, "4 Coins", {
        fontSize: "18px",
        color: "#FFFFFF",
      })
    );
    this.add(
      this.scene.add.text(80, 440, "6 Coins", {
        fontSize: "18px",
        color: "#FFFFFF",
      })
    );
    ...
    this.color.on("pointerup", () => {
      if ((this.scene as GameScene).coins >= 4) {
        ...
        (this.scene as GameScene).dragAndDrop?.dragColors["red"].setVisible(
          true
        );

        (this.scene as GameScene).dragAndDrop?.dragColors["purple"].setVisible(
          true
        );
        (this.scene as GameScene).dragAndDrop?.SetcolorsGot();
        this.add(
          this.scene.add.text(65, 175, "SOLD OUT!", {
            fontSize: "24px",
            color: "#FFFFFF",
          })
        );
    ...
    this.hats.on("pointerup", () => {
      if ((this.scene as GameScene).coins >= 6) {
        ...
        (this.scene as GameScene).dragAndDrop?.dragHats["visor"].setVisible(
          true
        );
        (this.scene as GameScene).dragAndDrop?.SethatsGot();
        this.add(
          this.scene.add.text(65, 340, "SOLD OUT!", {
            fontSize: "24px",
            color: "#ff0303",
          })
        );
